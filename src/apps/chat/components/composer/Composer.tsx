...
export function Composer(props: {
  ...
  const handleSendAction = React.useCallback(async (_chatModeId: ChatModeId, composerText: string): Promise<boolean> => {
    ...
    const cost = creditCosts[_chatModeId as keyof typeof creditCosts];
    if (!cost) {
      console.error('Invalid chat mode ID');
      return false;
    }
    ...
    const tokensUsed = tokensComposer; // Use the calculated tokensComposer
    console.log("Tokens Composer:", tokensComposer);
    console.log("Tokens Used:", tokensUsed);

    let adjustedTokens = tokensUsed;
    if (adjustedTokens > 0) {
        adjustedTokens += 4; // Adjust based on model requirements if necessary
    }

    const newTokens = user.tokens - adjustedTokens; // Deduct tokens from user's total
 ...
 // Secondary buttons
  ...
  const handleCallClicked = React.useCallback(async () => {
    if (!composeText || !props.conversationId)return;
    if (!user || user.credits < creditCosts.callFeature) {
       alert('Not enough credits to use Imagine');
      return;
    }

    const newCredits = user.credits - creditCosts.callFeature;
    try {
      // Deduct credits on the server-side
  ...
  const handleTextImagineClicked = React.useCallback(async() => {
    if (!composeText || !props.conversationId)return;
    if (!user || user.credits < creditCosts.textImagine) {
      alert('Not enough credits to use Imagine');
      return;
    }
    const newCredits = user.credits - creditCosts.textImagine;
    try {
      // Deduct credits on the server-side
      const response = await fetch('/api/updateUserdata', {
          method: 'POST',
  ...
  const handleTextareaKeyDown = React.useCallback(async (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Enter') {
      ...
      if (e.altKey) {
        const result = await handleSendAction('append-user', composeText);
          if (result) touchAltEnter();
        return e.preventDefault();
      }

      // Ctrl (Windows) or Command (Mac) + Enter: send for beaming
      if ((isMacUser && e.metaKey && !e.ctrlKey) || (!isMacUser && e.ctrlKey && !e.metaKey)) {
      const result = await handleSendAction('generate-text-beam', composeText);
          if (result) touchCtrlEnter();
        return e.preventDefault();
      }
      ...
      if (enterIsNewline ? e.shiftKey : !e.shiftKey) {
        if (!assistantAbortible)  {
          const result = await handleSendAction(chatModeId, composeText);
        if (result) {
